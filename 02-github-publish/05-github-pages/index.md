...
前章までで「pushすると `gh-pages` ブランチに自動でビルド結果が送られる」状態になりました。 最後に、「そのブランチの内容を世界に公開する」スイッチを入れます。

## 5-1. ブラウザの画面から設定する場合

1. リポジトリページを開く → **Settings** タブをクリック
1. 左メニューの **Pages** をクリック
1. 「Build and deployment」の **Source** を `Deploy from a branch` にする
1. **Branch** を `gh-pages` / `/ (root)` にして **Save**
...
- `gh-pages` ブランチが実際に存在するか確認（最初に1回 `mkdocs gh-deploy` または Actionsが成功している必要があります）
- 反映には数分かかることがあるので、少し待ってから再確認
- URLの末尾にスラッシュ `/` が必要なリポジトリ名サイト（`https://user.github.io/repo/`）と、 ユーザーサイト（`https://user.github.io/`）はパスの形が異なるので確認

Settingsに `Pages` メニューが見当たらない

プライベートリポジトリの場合、無料プランではGitHub Pagesが使えない場合があります。 リポジトリを `Public` にすると表示されます。
